...
void main() {
  ...
  test('Field Manual and accent-swap packs stay byte-identical', () {
    ...
    // The new tracking token must stay null everywhere except Woodland, or
    // every other theme's display type silently re-spaces.
    expect(resolved.displayTrackingEm, isNull);
    for (final theme in themeRegistry) {
      if (theme.id == 'woodland') continue;
      expect(FmSkin.fromTheme(theme).displayTrackingEm, isNull,
          reason: '${theme.id} picked up Woodland display tracking');
    }
  });
